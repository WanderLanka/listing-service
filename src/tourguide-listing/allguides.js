...
/**
 * GET / - List all guides
 * Proxies to guide-service with query parameters
 */
router.get('/', async (req, res, next) => {
  try {
    // Forward all query parameters
    const queryParams = new URLSearchParams(req.query).toString();
    const url = `${GUIDE_SERVICE_URL}/guide/list${queryParams ? `?${queryParams}` : ''}`;
    ...
    console.error('Error listing guides:', error.message);
    next(error);
  }
});
